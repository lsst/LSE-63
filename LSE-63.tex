...
\item Large data volume and data rate which prevents us from relying solely on deployment of traditional
...
\item Critical sensitivity of many science programs to very small systematic errors. The needed system
...
\item Nearly real-time reporting of transient sources requires robust triggering and their
          rapid and reliable characterization. A real-time data quality monitoring system is required to
          support these tasks, to minimize false transients arising from flaws in the data.
...
techniques), and to be supported by modern data visualization tools.
...
the Level 1-3 structure of LSST data products.
...
\section{General Desiderata for the LSST DQAF Design and Implementation}

A number of desirable design and performance features are common to all four DQAF levels:
...
\item Availability of all tools employed in a particular DQAF level to all other DQAF levels (with
          possible exceptions applying to Level 3 QA). In particular, it is highly desirable for the same QA tests to
        be available in the production context and in the software development context (i.e., in Level 0 QA).
...
\item Ability to store and export the results of DQAF analysis, including a user-friendly interface.
...
between individual measurements and their average values by CCD coordinates).  In general, the relevant
...
The DQAF must support such statistical analyses on all the spatial and temporal sclaes of the survey.
